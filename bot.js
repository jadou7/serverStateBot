...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("EaglesRP Statut du serveur", "https://cdn.discordapp.com/attachments/698213707057987648/704645475185197146/Logo_EaglesRP_avec_ecriture2.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ EaglesRP' : '⚫ EaglesRP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nComment rejoindre le serveur ?','Pour rejoindre le serveur depuis le menu direct connect taper play.eaglesrp.cf ou alors depuis le menu F8 taper connect play.eaglesrp.cf.\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('Joueur en ligne',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('EaglesRP',{'url':'https://www.twitch.tv/RoqueTV','type':'PLAYING'});
